...
trial(display_element, trial) {
  ...
  const disable_buttons = () => {
    const btns = document.querySelectorAll(
      ".jspsych-audio-betty-response-button"
    );
    btns.forEach((btn) => {
  ...
  const setupTrial = () => {
    ...
    let buttons = [];
    if (
      Array.isArray(trial.button_html) &&
      trial.button_html.length === trial.choices.length
    ) {
    ...
    // Enable or disable buttons based on trial parameters
